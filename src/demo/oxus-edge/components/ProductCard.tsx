...
interface ProductCardProps {
  ...
  /** Optional category chip shown below the title (e.g., family name). */
  category?: { name: string; slug: string } | null;
  /** If provided, the chip becomes a button that fires this instead of navigating. */
...
export default function ProductCard({ product, category, onCategoryClick }: ProductCardProps) {
  ...
  // Coming-soon is an explicit data flag. Optionally auto-clears after releaseAt passes.
  ...
  return (
    <div className="group/card">
      <Link href={`/product/${product.id}`} className="block">
        {/* Image area — `group/img` scopes the hover-swap + Quick Add to image only */}
        <div className="group/img aspect-[3/4] relative overflow-hidden bg-bg-medium">
          ...
          {soldOut && !comingSoon && (
            <div className="absolute inset-0 bg-bg-deep/40 flex items-center justify-center">
              <span className="font-display text-sm font-semibold tracking-[0.18em] uppercase text-white/90">
                Sold Out
              </span>
            </div>
          )}
          ...
          {comingSoon && (
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-black/10 flex items-center justify-center">
              <div className="text-center">
                <div className="w-10 h-px bg-white/50 mx-auto mb-3" />
                <span className="font-display text-[0.95rem] font-semibold tracking-[0.3em] uppercase text-white block">
                  Coming Soon
                </span>
                {releaseLabel && (
                  <span className="block mt-2 text-[0.62rem] tracking-[0.2em] uppercase text-white/70 font-display tabular-nums">
                    Drops {releaseLabel}
                  </span>
                )}
                <div className="w-10 h-px bg-white/50 mx-auto mt-3" />
              </div>
            </div>
          )}
          ...
          {/* Quick Add — hidden for soldOut and coming soon, only triggers on IMAGE hover */}
          {!soldOut && !comingSoon && (
            <button
              onClick={(e) => {
                e.preventDefault();
        ...
        <div className="pt-4 relative">
          <span className="absolute top-0 left-0 right-0 h-px bg-ember scale-x-0 origin-right group-hover/card:scale-x-100 group-hover/card:origin-left transition-transform duration-[350ms]" style={{ transitionTimingFunction: "cubic-bezier(0.16,1,0.3,1)" }} />
          <h3 className="font-display text-[0.9rem] font-medium tracking-[0.02em] leading-snug group-hover/card:text-ember transition-colors duration-200">
            {product.name}
          </h3>
        </div>
      </Link>

      {/* Category chip — outside the Link so a <button> can live here without nesting a button in an anchor */}
      {category && (
        <div className="mt-1">
          {onCategoryClick ? (
            <button
              ...
              className="inline-flex items-center gap-1 text-[0.62rem] font-display tracking-[0.14em] uppercase text-text-muted hover:text-ember transition-colors cursor-pointer"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-1 text-[0.62rem] font-display tracking-[0.14em] uppercase text-text-muted hover:text-ember transition-colors"
            >
      ...
      {/* Price + swatches — OUTSIDE `group/img`, so hover here does NOT swap the image */}
      <div className="pt-1.5 pb-2">
        <div className="flex items-center justify-between gap-3">
          ...
          {hasVariants && (
            <div className="flex items-center gap-1.5" role="radiogroup" aria-label="Handle color">
              {product.variants!.map((v, i) => (
                ...
                >
                  <span
                    className={`w-5 h-5 rounded-full border-[1.5px] transition-all duration-200 ${
                  ...
                  {!v.inStock && (
                    <span
                      ...
                      aria-hidden="true"
                    >
                      <span className="block w-7 h-[1.5px] bg-white/70 rotate-45" />
                    </span>
                  )}
                </button>
              ))}
        ...
        {/* Selected variant name — no redundant "Color:" prefix; swatches speak for themselves */}
